...
# no hooks/ simply no-ops instead of blocking commits.
#
...
installed=0
for src in "$ROOT"/hooks/*; do
  [ -f "$src" ] || continue
  name="$(basename "$src")"
  dest="$HOOK_DIR/$name"
  cat > "$dest" <<'SHIM'
#!/usr/bin/env bash
# Generated by lib/install-hooks.sh — do not edit. The real hook is
# hooks/<name> in the working tree; this only delegates to it.
set -euo pipefail
ROOT="$(git rev-parse --show-toplevel)"
HOOK="$ROOT/hooks/$(basename "$0")"
[ -x "$HOOK" ] || exit 0
exec "$HOOK" "$@"
SHIM
  chmod +x "$dest"
...
for src in "$ROOT"/hooks/*; do
  ...
  name="$(basename "$src")"
  if [ ! -x "$src" ]; then
    echo "install-hooks: FAIL hooks/$name is not executable (chmod +x it and commit the mode)." >&2
    fail=1
  fi
...
if [ -f "$ROOT/hooks/post-checkout" ]; then
