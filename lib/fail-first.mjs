 ...
 * catch: a key-surface guard whose nested check matched a leaf's last segment
 * ANYWHERE in the file, so `trace: { run: 'on' }` satisfied `timeouts.run`. It
 * reported 3 of 4 missing keys, hid one of the exact cases it was written for,
 * and it would have gone red on the unfixed file all the same. Nothing here
 * would have noticed. Reading the assertion is still a human's job.
 *
 ...
 * DECLARATION
 * -----------
 * A commit declares a guard in the changelog.d/ fragment it already adds. It
 * is opt-in and there is no inference from the diff: intent cannot be read off
 ...
 *   date: 2026-08-05
 *   issue: i-example
 *   title: "fix(config): flagKeys mapped a flag no command declares"
 *   guard-test: go test ./internal/projectconfig -run '^TestFlagKeysAreRealRunFlags$'
 *   guard-cwd: janus-bridge
 *   guard-revert: janus-bridge/internal/projectconfig/resolve.go
 *   guard-red: cmd/janus/run.go declares no such flag
 *   ---
 *
 ...
 * for a local-first repo may be never. Six fragments shipped an unrunnable spec
 * that way on 2026-08-25 while --lint reported all 93 OK.
 *
 ...
 * one reason: tests/core/fail-first.test.ts builds a throwaway repo with a real
 * commit that really declares a guard, and runs this script against it. Without
 * the seam the only thing exercisable end to end is `--try`, and the path CI
 * actually takes — fragment -> declaration -> revert -> two runs -> exit code —
 * would ship untested.
 */
...
/** Paths whose revert would take the guard itself away, making the check vacuous. */
export function isTestPath(p) {
  return (
    p.startsWith('tests/') ||
    p.includes('/testdata/') ||
    /_test\.go$/.test(p) ||
    /\.(test|spec|fuzz\.test)\.(ts|tsx|js|mjs)$/.test(p)
  );
}
...
export function isNonCodePath(p) {
  return (
    p.startsWith(FRAGMENT_PREFIX) ||
    p.startsWith('docs/') ||
    p.startsWith('planning-docs/') ||
    p.startsWith('.github/') ||
    p.startsWith('.claude/') ||
    p.endsWith('.md')
  );
...
/**
 * Run `npm run gen` inside the extracted tree.
 *
 * `git archive` carries TRACKED files only, and several generated modules are
 * gitignored on purpose — `auspex-md.generated.ts`, `janus-md.generated.ts`,
 * `protocol.generated.ts`. Anything importing one fails to RESOLVE in the
 * archive, which surfaces as `control-not-green` on a guard that passes
 * perfectly well in the working tree.
 *
 * That is worse than it sounds: the failure is indistinguishable from "your
 * test is broken", so the natural response is to add `guard-skip` — and every
 * JS guard whose module graph touches a generated file would drift into being
 * skipped, leaving this job quietly Go-only. Found landing the auspex render
 * guard (b9bb7a8), whose only sin was importing `auspex-md.generated.js`.
 *
 * Best-effort: a repo without the script, or a gen that fails, still gets the
 * run — it just fails the same way it would have anyway.
 */
...
function runTest(cmd, cwd, timeout) {
  const r = spawnSync('sh', ['-c', cmd], {
    ...
    env: {
      ...
      // a file the test opens at runtime. Caught during development on 49ea387:
      // the control run cached a PASS for TestExampleConfigDocumentsEveryKey and
...
export function checkGuard(sha, decl, { timeout = DEFAULT_TIMEOUT_MS, log = console.log } = {}) {
  ...
  for (const { path: p } of specs) {
    ...
    //
    // Found on i-wire-commune-to-cascade, whose guard lives in
    // commune_trace_retention_test.go while the revert legitimately needed
    // commune_chrome_test.go and commune_config_test.go. The author's by-hand
    // A/B had done exactly that and was sound; only the declaration could not
    // express it.
