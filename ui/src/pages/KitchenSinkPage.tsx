...
import { Sidebar, type SidebarSectionGroup } from "../components/Sidebar";
...
//
// Route: /kitchen-sink (see App.tsx). Not linked from the sidebar's
// Categories list — it's a QA fixture, not a product page.
// ============================================================================
...
const THREAD_ROUNDS: ThreadRound[] = [
  ...
  },
  {
    ...
    label: "Follow-up",
    discuss:
      "One open thread left: does the kitchen sink need its own /kitchen-sink entry in the sidebar's Categories list, or is the route enough?",
  },
];
...
export function KitchenSinkPage() {
  ...
  const sections = useMemo<SidebarSectionGroup[]>(() => {
    ...
    return [{ label: "Kitchen Sink", items }];
  }, [responses]);

  return (
    <>
      <Sidebar sections={sections} currentSectionId={currentSectionId} onSelectSection={selectSection} />
      <main>
